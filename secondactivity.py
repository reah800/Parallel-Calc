...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Creative Grade Computing System")

    tk.Label(root, text="Enter grades (space-separated):").pack(pady=5)
    ...
    tk.Button(root, text="Run Multithreading", command=lambda: run_method("thread", grades_entry, tree, progress)).pack(pady=5)
    tk.Button(root, text="Run Multiprocessing", command=lambda: run_method("process", grades_entry, tree, progress)).pack(pady=5)
